...
function CategoryMain() {
    ...
    //     })
        ...
        useEffect(() => {
            ...
            .then((res) => {
                setData(res.data)
                setFilterData(fileteredData(res.data,'brand','Fatima Khadija'))
                setLoader(false)
            })
    ...
    // let filterData = data.length == 0 ? 0 : data.filter((fil) => fil.brand == 'Fatima Khadija')
    return (
        <>
            <Header />
                    ...
                    return (
                        <>
                            <Grid item xs={6} md={3} key={e._id}>
                                <Cards cardData={e} onClick1={() => addToCard(e)} onClick={() => viewDetails(e)} notnew={true} pic={e.imageUrl1} pic2={e.imageUrl2} title={e.title} price={e.orignalPrice} price2={e.dorignalPrice}/>
                            </Grid>
                        </>
                    )
                }) : <NoDataFound />}
