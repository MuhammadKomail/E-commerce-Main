...
function CategoryMain() {
    ...
    //     })
        ...
        useEffect(() => {
            ...
            .then((res) => {
                setData(res.data)
                setFilterData(fileteredData(res.data,'brand','Zarif'))
                setLoader(false)
            })
                    ...
                    return (
                        <>
                            <Grid item xs={6} md={3} key={e._id}>
                                <Cards onClick1={() => addToCard(e)} onClick={() => viewDetails(e)} notnew={true} pic={e.imageUrl1} pic2={e.imageUrl2} title={e.title} price={e.orignalPrice} price2={e.dorignalPrice}/>
                            </Grid>
                        </>
                    )
                }) : <NoDataFound />}
